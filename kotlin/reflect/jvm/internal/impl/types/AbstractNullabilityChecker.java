            ...
            while (!supertypesDeque.isEmpty()) {
                ...
                if (supertypesSet.add(current)) {
                    ...
                    if (none == null) {
                        ...
                        while (it.hasNext()) {
                            SimpleTypeMarker mo2650transformType = none.mo2650transformType(typeCheckerState, it.next());
                            if ((typeSystemContext.isClassType(mo2650transformType) && !typeSystemContext.isMarkedNullable(mo2650transformType)) || typeSystemContext.isDefinitelyNotNullType(mo2650transformType)) {
                                typeCheckerState.clear();
                            } else {
                                supertypesDeque.add(mo2650transformType);
                            }
                        }
                    }
                }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker mo2650transformType = supertypesPolicy.mo2650transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, mo2650transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(mo2650transformType);
