...
public class Processor implements ExecutionListener, ForegroundProcessor {
    ...
    public boolean startWork(StartStopToken startStopToken, WorkerParameters.RuntimeExtras runtimeExtras) {
        ...
        WorkSpec workSpec = (WorkSpec) this.mWorkDatabase.runInTransaction(new Callable() { // from class: androidx.work.impl.Processor$$ExternalSyntheticLambda0
            ...
            public final Object call() {
                return Processor.this.m236lambda$startWork$0$androidxworkimplProcessor(arrayList, workSpecId);
            }
        });
        if (workSpec == null) {
    ...
    public /* synthetic */ WorkSpec m236lambda$startWork$0$androidxworkimplProcessor(ArrayList arrayList, String str) throws Exception {
    ...
    public void m235lambda$runOnExecuted$1$androidxworkimplProcessor(final WorkGenerationalId id, boolean needsReschedule) {
        synchronized (this.mLock) {
            ...
            Logger.get().debug(TAG, getClass().getSimpleName() + MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR + id.getWorkSpecId() + " executed; reschedule = " + needsReschedule);
            Iterator<ExecutionListener> it = this.mOuterListeners.iterator();
            while (it.hasNext()) {
                it.next().m235lambda$runOnExecuted$1$androidxworkimplProcessor(id, needsReschedule);
            }
        }
    }
    ...
    private void runOnExecuted(final WorkGenerationalId id, final boolean needsReschedule) {
        this.mWorkTaskExecutor.getMainThreadExecutor().execute(new Runnable() { // from class: androidx.work.impl.Processor$$ExternalSyntheticLambda1
            ...
            public final void run() {
                Processor.this.m235lambda$runOnExecuted$1$androidxworkimplProcessor(id, needsReschedule);
            }
        });
    }
    ...
    public static class FutureListener implements Runnable {
        ...
        public void run() {
            ...
            this.mExecutionListener.m235lambda$runOnExecuted$1$androidxworkimplProcessor(this.mWorkGenerationalId, z);
        }
    }
}
