...
final class DashMediaPeriod implements MediaPeriod, SequenceableLoader.Callback<ChunkSampleStream<DashChunkSource>>, ChunkSampleStream.ReleaseCallback<DashChunkSource> {
    ...
    private static int[][] getGroupedAdaptationSetIndices(List<AdaptationSet> list) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            ...
            if (i == i3 && (findAdaptationSetSwitchingProperty = findAdaptationSetSwitchingProperty(adaptationSet.supplementalProperties)) != null) {
                for (String str : Util.split(findAdaptationSetSwitchingProperty.value, f.a)) {
    ...
    private static void buildManifestEventTrackGroupInfos(List<EventStream> list, TrackGroup[] trackGroupArr, TrackGroupInfo[] trackGroupInfoArr, int i) {
        ...
        while (i2 < list.size()) {
            ...
            trackGroupArr[i] = new TrackGroup(eventStream.id() + Constants.SUBS_ID_BASE_PLAN_ID_SEPARATOR + i2, new Format.Builder().setId(eventStream.id()).setSampleMimeType(MimeTypes.APPLICATION_EMSG).build());
    ...
    private static Format[] parseClosedCaptionDescriptor(Descriptor descriptor, Pattern pattern, Format format) {
        ...
        for (int i = 0; i < split.length; i++) {
            ...
            formatArr[i] = format.buildUpon().setId(format.id + Constants.SUBS_ID_BASE_PLAN_ID_SEPARATOR + parseInt).setAccessibilityChannel(parseInt).setLanguage(matcher.group(2)).build();
        }
        return formatArr;
    }
