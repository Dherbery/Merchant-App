...
public final class RtspAuthenticationInfo {
    ...
    private String getDigestAuthorizationHeaderValue(RtspMessageUtil.RtspAuthUserInfo rtspAuthUserInfo, Uri uri, int i) throws ParserException {
        try {
            ...
            String hexString = Util.toHexString(messageDigest.digest(RtspMessageUtil.getStringBytes(rtspAuthUserInfo.username + Constants.SUBS_ID_BASE_PLAN_ID_SEPARATOR + this.realm + Constants.SUBS_ID_BASE_PLAN_ID_SEPARATOR + rtspAuthUserInfo.password)));
            StringBuilder sb = new StringBuilder();
            sb.append(methodString);
            sb.append(Constants.SUBS_ID_BASE_PLAN_ID_SEPARATOR);
            sb.append(uri);
            String hexString2 = Util.toHexString(messageDigest.digest(RtspMessageUtil.getStringBytes(hexString + Constants.SUBS_ID_BASE_PLAN_ID_SEPARATOR + this.nonce + Constants.SUBS_ID_BASE_PLAN_ID_SEPARATOR + Util.toHexString(messageDigest.digest(RtspMessageUtil.getStringBytes(sb.toString()))))));
